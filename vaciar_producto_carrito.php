...
<?php   
    ...
    $conexion = conectar();
    $result = oci_parse($conexion,"begin vaciar_producto_carrito(:PARAM); end;");
    oci_bind_by_name($result,':PARAM',$_GET['id']);
    //echo $sql;
    //Verifica y ejecuta consulta
    if (oci_execute($result) == TRUE){
        echo "<script type=\"text/javascript\">alert(\"Producto eliminado de carritos exitosamente\");</script>";
